...
let all = 
  frame [ "Pop2000" => round (pop2000 / 1.0e6)
          "Pop2010" => round (pop2010 / 1.0e6) ]
(*** include-value:all ***)
...
indicators:
*)
all?GDP <- [ for c in wb.Countries -> c.Indicators.``GDP (current US$)``.[2000] ]
all?Growth <- [ for c in wb.Countries -> c.Indicators.``GDP per capita growth (annual %)``.[2000] ]
all?Emissions <- [ for c in wb.Countries -> c.Indicators.``CO2 emissions (kg per PPP $ of GDP)``.[2000] ]
all?Gender <- [ for c in wb.Countries -> c.Indicators.``Employment to population ratio, 15+, female (%)``.[2000] ]
(**
