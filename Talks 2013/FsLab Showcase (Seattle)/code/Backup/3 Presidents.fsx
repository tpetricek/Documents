...
let ed = endDebt |> Frame.dropSparseRows
...
aligned.Rows
|> Series.observations
|> Seq.map (fun (year, row) -> 
  let lbl = sprintf "(%d) %s" (row.GetAs "Start") (row.GetAs "President")
  let debt = row?Debt
  let party = row.GetAs<string>("Party")
  string year, debt, party)
|> colorBar
