  ...
  Agent.Start(fun inbox -> async {
    ...
    while true do
      ...
      | Get(key, channel) ->
          ...
          | true, value -> channel.Reply(Some value)
          | _ -> channel.Reply(None) })

// ----------------------------------------------------------------------------
// Completely simple proxy 

let proxy1 = 
  HttpServer.Start("http://localhost:8082/", fun ctx -> async {
    let url = root + ctx.Request.Url.PathAndQuery
    let! res = cache.PostAndAsyncReply(fun c -> Get(url, c))
    match res with 
    | Some html ->
    ...
    | None ->
        ...
        cache.Post(Store(url, html.Replace("<title>", "<title>Cached: ")))
        do! ctx.Response.AsyncReply(html) })
