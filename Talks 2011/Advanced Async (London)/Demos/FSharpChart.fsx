...
module ChartData = 
    ...
    module internal Internal = 
        ...
        let internal bindObservable (chart:Chart) (series:Series) maxPoints values adder = 
            series.Points.Clear()
            let rec disposer = EventHandler(fun _ _ ->
              (disp:IDisposable).Dispose() )
            and disp = 
            ...
            chart.Disposed.AddHandler(disposer)
...
module internal ChartFormUtilities = 
    ...
    let typesToClone = 
        [ typeof<System.Windows.Forms.DataVisualization.Charting.LabelStyle>; typeof<Axis>; typeof<Grid>; typeof<TickMark>
                        ...
                        if typesToClone |> Seq.exists ((=) p.PropertyType) then
                            ...
                            if p.PropertyType.IsValueType || typesToCopy |> Seq.exists ((=) p.PropertyType) then
                                if p.GetSetMethod().GetParameters().Length <= 1 then
                                    //printfn "Setting %s.%s" ty.Name p.Name
