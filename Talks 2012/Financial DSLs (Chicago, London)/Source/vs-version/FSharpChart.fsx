    ...
    module internal Internal = 
        ...
        let internal bindObservable (chart:Chart) (series:Series) maxPoints values adder = 
            let work () =
                  ...
                  values |> Observable.subscribe (fun v ->
                      let op () = 
                          try
                              ...
                              // Update maximum & minimum (does not work automatically)
                              let area = chart.ChartAreas.[series.ChartArea]
                              area.AxisX.Minimum <- series.Points.[0].XValue
                              area.AxisX.Maximum <- series.Points.[series.Points.Count - 1].XValue
                              let min = series.Points |> Seq.map (fun pt -> pt.YValues.[0]) |> Seq.min
                              let max = series.Points |> Seq.map (fun pt -> pt.YValues.[0]) |> Seq.max
                              //printfn "min, max: %A" (min, max)
                              if max > min then
                                let minSp = min - (max - min) / 5.0
                                let maxSp = max + (max - min) / 5.0
                                //printfn "%A" (minSp, maxSp)
                                area.AxisY.Minimum <- minSp
                                area.AxisY.Maximum <- maxSp
                                //printfn "done"
              ...
              ()
            if chart.InvokeRequired then
                chart.Invoke(Action(work)) |> ignore
            else 
                work()
