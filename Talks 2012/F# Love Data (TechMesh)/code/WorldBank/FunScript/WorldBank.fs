...
type WorldBankResponse = FSharp.Web.JsonProvider<"FunScript/worldbank.json">
    ...
    static member AsyncGetIndicator(country:WorldBankIndicators, indicator:string) : Async<seq<int * float>> =
      Async.FromContinuations(fun (cont, econt, ccont) ->
        ...
        getJSON(url + "&prefix=?", fun json ->
          ...
          let res =
            data.GetCollection() |> Seq.ofArray |> Seq.choose (fun v ->
              match v.Value with
              | None -> None
              | Some value -> Some(int v.Date, float value))
          cont res ))

open Runtime
open FSharp.Data.WorldBankProvider
