...
type h = TypeScript.Api<"Types/highcharts.d.ts">
...
let main() = 
  ...
  let render () = async {
    let opts = h._HighchartsOptions()
    opts.chart <- h._HighchartsChartOptions(renderTo = "chart", ``type`` = "line")
    opts.title <- h._HighchartsTitleOptions(text = "School enrollment, tertiary (% gross)")
    opts.series <- [| |]
    
    // Create series we want to render
    let series = ref []
    for country, check in infos do
      if check.is(":checked") then
        let! vals = country.Indicators.``School enrollment, tertiary (% gross)``
        let data = vals |> Seq.map (fun (k, v) -> [| number k; number v |]) |> Array.ofSeq
        opts.series.push(h._HighchartsSeriesOptions(data=data, name=country.Name))
